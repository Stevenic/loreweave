 ...
 */
export function renderScene(ctx, scene, registry, createCanvas, state) {
	...
	for (const layer of layers) {
		...
		if (layerType === 'tilemap' && layer.tilemap) {
			renderTilemapLayer(ctx, layer.tilemap, registry, createCanvas);
		} else if (layerType === 'items' && layer.items) {
			renderItemsLayer(ctx, layer, registry, createCanvas, time, state);
		}
		ctx.restore();
	}
}
/** Render a tilemap layer. */
function renderTilemapLayer(ctx, tilemapName, registry, createCanvas) {
	const tilemap = registry.tilemaps.get(tilemapName);
	if (!tilemap) return;
	const tileset = registry.tilesets.get(tilemap.tileset);
	if (!tileset) return;
	const entries = resolvePaletteEntries(tileset.palette, registry.palettes);
...
function renderItemsLayer(ctx, layer, registry, createCanvas, time, state) {
	...
	for (const item of items) {
		renderSceneItem(ctx, item, registry, createCanvas, time, state, itemById);
	}
}
/** Render a single scene item. */
function renderSceneItem(ctx, item, registry, createCanvas, time, state, itemById) {
...
export function renderSprite(ctx, sprite, createCanvas, options) {
	...
	const x = options?.x ?? 0;
	const y = options?.y ?? 0;
	const imageData = rasterizeSpriteFrame(sprite, frame, paletteRegistry);
