 ...
 */
import { DEFAULT_ANIMATED_PROPERTIES, sampleClip } from './animation.js';
...
function resolvePaletteEntries(palette, registry, overrides) {
    let entries;
    if (typeof palette === 'string') {
        const found = registry.get(palette);
        if (!found)
            throw new Error(`Palette '${palette}' not found in registry`);
...
function findTransparentKey(entries) {
 ...
 */
export function rasterizeSpriteFrame(sprite, frameIndex, paletteRegistry, paletteOverrides) {
    ...
    if (sprite.layers && sprite.layers.length > 0) {
        ...
        for (const layer of sprite.layers) {
            const allFrames = decodeAllFrames(sprite.encoding, layer.pixels, sprite.width, sprite.height, frameCount);
        ...
        const allFrames = decodeAllFrames(sprite.encoding, sprite.pixels, sprite.width, sprite.height, frameCount);
 ...
 */
export function rasterizeTile(tile, tileWidth, tileHeight, entries) {
 ...
 */
export function drawImageData(ctx, imageData, x, y, createCanvas, options) {
    ...
    if (flipX)
        ctx.translate(-imageData.width, 0);
    if (flipY)
 ...
 */
export function renderScene(ctx, scene, registry, createCanvas, state, options) {
...
function renderTilemapLayer(ctx, tilemapName, registry, createCanvas, targetPalette) {
    ...
    for (let gy = 0; gy < tilemap.gridHeight; gy++) {
        ...
        for (let gx = 0; gx < tilemap.gridWidth; gx++) {
            ...
            const ref = typeof legendEntry === 'object' ? legendEntry : null;
...
function renderItemsLayer(ctx, layer, registry, createCanvas, time, state, targetPalette, referencePpu) {
    if (!layer.items)
        return;
...
function renderSceneItem(ctx, item, registry, createCanvas, time, state, itemById, targetPalette, referencePpu) {
    ...
    // Resolve animation
    let animProps = { ...DEFAULT_ANIMATED_PROPERTIES };
    ...
    if (item.emitter && state) {
        ...
        renderEmitterParticles(ctx, drawX, drawY, item.emitter, emitterId, sprite, registry, createCanvas, state, referencePpu);
    }
}
/** Draw a simple shadow ellipse for depth anchoring. */
function drawShadow(ctx, x, groundY, width, scale) {
    ctx.save();
    ctx.globalAlpha *= 0.3;
...
function renderEmitterParticles(ctx, originX, originY, config, emitterId, sprite, registry, createCanvas, state, referencePpu) {
    ...
    for (const particle of emitter.particles) {
        drawImageData(ctx, imageData, originX + particle.x * emitterPpuFactor, originY + particle.y * emitterPpuFactor, createCanvas, {
 ...
 */
export function renderSprite(ctx, sprite, createCanvas, options) {
    ctx.imageSmoothingEnabled = false;
