...
const ALLOWED_PALETTE_KEYS = new Set('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz!@#$%&*+=?'.split(''));
...
function resolvePaletteEntries(palette, registry) {
...
function validatePaletteKeys(decodedRows, entries, prefix) {
    const errors = [];
...
export function validatePalette(palette) {
    ...
    for (const [key, value] of Object.entries(palette.entries)) {
        if (key.length !== 1) {
    ...
    if (palette.ramps) {
        for (const [rampName, keys] of Object.entries(palette.ramps)) {
...
export function validateSprite(sprite, registry) {
    const errors = [];
    const warnings = [];
    ...
    if (sprite.regions) {
        for (const [name, region] of Object.entries(sprite.regions)) {
            if (region.x < 0 ||
                region.y < 0 ||
    ...
    if (sprite.attachments) {
        for (const [name, att] of Object.entries(sprite.attachments)) {
...
export function validateTileset(tileset, registry) {
    const errors = [];
    ...
    const results = [];
    for (const [name, tile] of Object.entries(tileset.tiles)) {
        try {
            const decoded = decodePixelRows(tile.encoding, tile.pixels, tileset.tileWidth, tileset.tileHeight);
            ...
            results.push(fail(`Tile '${name}': ${e.message}`));
        }
    }
...
export function validateTilemap(tilemap, tilesetRegistry) {
    const errors = [];
    const warnings = [];
    ...
    if (tilesetRegistry) {
        ...
        if (!tileset) {
            ...
            for (const [key, ref] of Object.entries(tilemap.legend)) {
                const tileName = typeof ref === 'string' ? ref : ref.tile;
                if (!(tileName in tileset.tiles)) {
                    errors.push(`Legend '${key}': tile '${tileName}' not found in tileset '${tilemap.tileset}'`);
                }
            }
        }
    }
...
// ─── Scene Validation ───
const VALID_BLEND_MODES = ['normal', 'multiply', 'screen', 'overlay', 'additive'];
...
export function validateScene(scene) {
    ...
    if (scene.canvas?.referencePpu !== undefined &&
            ...
            if (layer.items) {
                for (let j = 0; j < layer.items.length; j++) {
                    ...
                    if (item.palette && typeof item.palette === 'object') {
                        const paletteResult = validatePalette(item.palette);
                        if (!paletteResult.valid) {
                            errors.push(...paletteResult.errors.map((e) => `Layer '${layer.name}', item ${j} palette: ${e}`));
                        }
                    }
                    ...
                    if (item.emitter) {
                        ...
                        if (!emitterResult.valid) {
                            errors.push(...emitterResult.errors.map((e) => `Layer '${layer.name}', item ${j} emitter: ${e}`));
                        }
                    }
                }
            }
    ...
    for (const ref of attachRefs) {
        if (!itemIds.has(ref.parent)) {
            warnings.push(`Layer ${ref.layerIdx}, item ${ref.itemIdx}: attach parent '${ref.parent}' not found`);
        }
    }
    const result = errors.length > 0 ? fail(...errors) : ok();
...
export function validateEmitter(emitter) {
    const errors = [];
